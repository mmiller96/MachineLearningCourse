...
# calculate error for different depth of layers and number of layers
# return error and parameters 
def calc_error_different_layers(layer1_num, layer2_num=None, layer3_num=None):
    ...
    cv_results = cross_validate(mbsgd, X_train, Y_train, cv=3, n_jobs=3) #n_jobs for parallel processing
...
path_folder = r'C:\Users\Data Miner\Desktop\Uni\MachineLearning\ex05\inverse_dynamics\Bilder'
download_sarcos()
...
F = Y_train.shape[1]
D = (X_train.shape[1],)
nodes_layer = np.array([30, 50, 70])

train_error_list, test_error_list, param_list = [], [], []
time_start = time.time()
# 3 hidden layers
for nodes1 in nodes_layer:
    for nodes2 in nodes_layer:
        for nodes3 in nodes_layer:
            train_error, test_error, param = calc_error_different_layers(nodes1, layer2_num=nodes2, layer3_num=nodes3)
...
for nodes1 in nodes_layer:
    for nodes2 in nodes_layer:
        train_error, test_error, param = calc_error_different_layers(nodes1, layer2_num=nodes2, layer3_num=None)
        train_error_list.append(train_error)
        test_error_list.append(test_error)
        param_list.append(param)
...
save_error_params(train_error_list, test_error_list, param_list, r'C:\Users\Markus Miller\Desktop\Uni\Machine Learning\ex05\inverse_dynamics\search_architecture_error1.csv')